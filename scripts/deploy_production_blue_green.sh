...
PROD_PUBLIC_HTTP_OPENAPI_URL="${PROD_PUBLIC_HTTP_OPENAPI_URL:?PROD_PUBLIC_HTTP_OPENAPI_URL is required}"
...
wait_for_public_blocked() {
  ...
  while [ "$elapsed" -lt "$timeout_seconds" ]; do
    ...
    echo "${label} returned HTTP ${status_code:-unknown}; waiting for block rule..."
    sleep "$interval_seconds"
    elapsed=$((elapsed + interval_seconds))
  done

  echo "${label} was not blocked at $url" >&2
  return 1
}
...
if ! wait_for_public_blocked "Public docs" "$public_docs_url" 30; then
  echo "Public docs was not blocked after switching traffic to port ${target_port}" >&2
  if [ -n "$current_backend_port" ] && [ "$current_backend_port" != "$target_port" ]; then
    install_production_nginx "$current_backend_port"
  fi
  exit 1
...
if ! wait_for_public_blocked "Public OpenAPI" "$public_openapi_url" 30; then
  echo "Public OpenAPI was not blocked after switching traffic to port ${target_port}" >&2
  if [ -n "$current_backend_port" ] && [ "$current_backend_port" != "$target_port" ]; then
    install_production_nginx "$current_backend_port"
  fi
  exit 1
