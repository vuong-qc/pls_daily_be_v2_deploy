...
class FormatContentGgChatAPI:
    ...
    @staticmethod
    def format_content_checkout(user_name: str, tasks: list[str], note: Optional[str]=None) -> dict:
        tz_vn = ZoneInfo(settings.TZ)
        now_vn = datetime.now(tz_vn)
        content = TextFormatEnum.CHECKOUT.format(time=now_vn.strftime("%d.%m.%Y %H:%M"), user=user_name)

        # Build HTML text cho Task
        if tasks:
            task_list_str = TextFormatEnum.NEWLINE.join(f'{TextFormatEnum.TASK_PREFIX}{task}' for task in tasks)
            task_lines = f"{TextFormatEnum.SUBTASK_HEADER}{TextFormatEnum.NEWLINE}{task_list_str}"
        else:
            task_lines = f"{TextFormatEnum.SUBTASK_HEADER}{TextFormatEnum.SPACE}{TextFormatEnum.TASK_EMPTY}"
