...
class ExceptionMasterFoundUserRoleOutScope(HTTPException):
    def __init__(self):
        super().__init__(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail='Master only find user with role: handler, interviewer, trainer'
        )
...
class ExceptionAdminCreateUserOutSCope(HTTPException):
    def __init__(self):
        super().__init__(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Amin only can create user with role: handler, manager, tasker"
        )
class ExceptionMasterUpdateUserOutScope(HTTPException):
    def __init__(self):
        super().__init__(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Master, admin only can update user with role: manager, handler, tasker"
        )
